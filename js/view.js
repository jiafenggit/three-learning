//加载房间
			function addRoom(){
				//创建三面墙和地板----------------------------------------------------
						// 创建好一个方块的材质 (第一面墙左边)
				 var cubeGeometry = new THREE.CubeGeometry(0.2,8,25.9);
				var materials = [];
				for (var i = 0; i < 6; ++i) {
	            ...
	            //创建墙脚面
						// 创建好一个方块的材质 (第一面墙脚面)
				var cubeGeometryK1 = new THREE.CubeGeometry(0.8,0.4,26);
				...
				var cubeK1 = new THREE.Mesh(cubeGeometryK1,
				    new THREE.MeshFaceMaterial(cubeMaterialK1));
	            cubeK1.position.x = -23;
	            cubeK1.position.y =8.2;
	            cubeK1.position.z = 0;
	            cubeK1.castShadow = true;
	            // 同样的，得指定它在坐标系中的位置。
	            scene.add(cubeK1);
	            	//创建几何体  第二面墙右边
				 var cubeGeometry2 = new THREE.CubeGeometry(0.2,8,25.9);
	            // 创建一个方块的几何形状
	       		 var materials2 = [];
				for (var i = 0; i < 6; ++i) {
	            ...
	            scene.add(cubeK1);
	            	...
	            	//创建几何体  第三面墙后边
				 var cubeGeometry3 = new THREE.CubeGeometry(46,8,0.2);
	            // 创建一个方块的几何形状
	            var materials3 = [];
				for (var i = 0; i < 6; ++i) {
					var pic = "img/huise.jpg";
	            ...
	            scene.add(cube3);
						...
						// 创建好一个方块的材质 (第一面墙脚面)
				var cubeGeometryK1 = new THREE.CubeGeometry(47,0.4,1);
				var cubeMaterialK1 = new THREE.MeshLambertMaterial({color:   0x7AC5CD});
				var cubeK1 = new THREE.Mesh(cubeGeometryK1,
				    new THREE.MeshFaceMaterial(cubeMaterialK1));
	            cubeK1.position.x = 0;
				...
				//创建几何体  地板
				 var cubeGeometry4 = new THREE.CubeGeometry(50,0.4,30);
	            // 创建一个方块的几何形状
	            var materials4 = [];
				for (var i = 0; i < 6; ++i) {
					...
					if(i==3 || i==2){
					        ...
					        map: THREE.ImageUtils.loadTexture('img/ds.jpg',
					            {}, function() {
					            renderer.render(scene, camera);
						        }), 
						    overdraw: true
